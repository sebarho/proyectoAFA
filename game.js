...
document.addEventListener('DOMContentLoaded', () => {
    ...
    loadAssets(allImagePaths, () => {
        ...
        let { characters, scenes, dialogueMatrix } = JSON.parse(JSON.stringify(gameData));
        ...
        Object.keys(characters).forEach(charKey => {
            initialInventories[charKey] = [];
        });
        initialInventories['Muñeco'].push('encendedor');
        ...
        const dependencies = {
            ...
            puzzles,
            dialogueMatrix,
            gameData, // Pass the original data for reference
        ...
        function handleVerbClick(verb) {
            const newAction = verb.toUpperCase().replace(' ', '_');
            if (gameState.actionState === newAction) {
                gameState.actionState = null;
            } else {
                gameState.actionState = newAction;
            }
        ...
        function handleActionClick(target, event, dependencies) {
            let actionResultText = "";
            let dialogueStarted = false;
            const { gameState, characters, scenes, puzzles } = dependencies;
            ...
            if (!dialogueStarted && !(actionResultText.includes("GANASTE"))) { //TODO: Cambiar esto. el fin del juego de hacerse con un script
                dependencies.dialogueText.textContent = actionResultText;
            }
        ...
        function getTargetAt(x, y, { scenes, characters, gameState }) {
            ...
            for (const target of allTargets) {
                const d = target.data;
                if (x >= d.x && x <= d.x + d.width && y >= d.y && y <= d.y + d.height) return target;
            }
            return null;
        }
