...
export const storyScripts = {
    'default': () => {
        ...
        switch (as.verb) {
            ...
            case "AGARRAR": {
                if (api.getItem(as.item.key) == false) {
                    api.runScript("no_puedo");
                }
                break;
            }
            case "USAR": {
            ...
            case "DAR": {
                const actor = api.getGameState().currentPlayer;
                const item = api.getGameState().actionState.item.key;
                const target = api.getGameState().actionState.target.key;
                if (actor === target) {
                    api.say("No tiene sentido darme algo a mí mismo.");
                    break;
                }
                if (api.getActor(target).playable) {
                    if (api.actorHasItem(actor, item)) {
                        api.removeItemFromActor(actor, item);
                        api.say(actor + ": Tomá esto.");
                        api.addItemToActor(target, item);
                        api.say(target + ": Gracias.");
                    } else {
                        api.say("Deberia agarrarlo primero.");
                    }
                } else {
                    api.say("Algo me dice que no lo haga.");
                }
                break;
            }
            case "HABLAR": {
    ...
    'no_puedo': () => {
        ...
        switch (n) {
            case 1:
                api.say("No entiendo qué querés hacer.");
                break;
            case 2:
            ...
            case 3:
                ...
                break;
            case 4:
                api.say("No sé cómo ayudarte con eso.");
                break;
            case 5:
                api.say("¿Estás seguro de eso?");
                break;
            case 6:
                api.say("Lo miro de todos los angulos pero no veo la forma de hacerlo.");
                break;
        }
    ...
    }, 'usar_vaso_vacio_coto_cola': () => { api.runScript('usar_coto_cola_vaso_vacio'); },
    'usar_botella_fernet_vaso_vacio': () => {
        api.say("Por suerte compraron una botella grande.");
        api.switchItem('vaso_vacio', 'vaso_fernet');
    }, 'usar_vaso_vacio_botella_fernet': () => { api.runScript('usar_botella_fernet_vaso_vacio'); },
    'usar_coto_cola_vaso_fernet': () => {
        api.say("soy cun crack haciendo tragos.");
        api.switchItem('vaso_fernet', 'vaso_preparado');
    }, 'usar_vaso_fernet_coto_cola': () => { api.runScript('usar_coto_cola_vaso_fernet'); },
    'usar_botella_fernet_vaso_coca': () => {
        api.say("Y por ultimo 30% de fernandito...");
        api.say("soy cun crack haciendo tragos.");
        api.switchItem('vaso_coca', 'vaso_preparado');
    }, 'usar_vaso_coca_botella_fernet': () => { api.runScript('usar_botella_fernet_vaso_coca'); },
    'agus_dar_encendedor': () => {
