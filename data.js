...
import { runActionScript,
 } from "./game.js";
import { gameState, getGameState } from "./runtime.js";
...
export const gameData = {
    ...
    characters: {
        ...
        'Bocha': { name: 'Darío', alias: 'El Bocha', description: "El Bocha: El Ausente.", x: 400, y: 150, width: 130, height: 200, 
    ...
    objects: {
        ...
        'salida_patio': { name: 'Salida al Patio', x: 780, y: 150, width: 20, height: 160, description: "Hacia el verde césped." },
        'salida_cocina': { name: 'Entrada a la Cocina', x: 0, y: 150, width: 20, height: 160, description: "A la cocina, donde nacen las ensaladas." },
        'pileta': { name: 'Pileta', x: 400, y: 250, width: 300, height: 100, color: '#0000FF', description: "Una pileta. Ideal para refrescarse." },
        'entrada_quincho': { name: 'Entrada al Quincho', x: 0, y: 150, width: 20, height: 160, description: "De vuelta a la acción." },
        ...
        'salida_quincho': { name: 'Salida al Quincho', x: 780, y: 150, width: 20, height: 160, description: "Volver con los muchachos." }
    },
    ...
    scenes: {
        'quincho': {
            ...
            characters: ['Pancho', 'Sebastian', 'Juan', 'JuanMa', 'Bocha', 'Ale', 'Federico', 'Rata', 'Pelu', 'Locura', 'Pol', 'Muñeco']
        },
        'patio': {
            name: 'Patio',
...
export const storyScripts = {
        'usar_pala_jardin_en_heladera': (dependencies) => {
            const { scenes } = dependencies;
        ...
        'usar_bolsa_carbon_en_parrilla':(dependencies) => {
            const parrilla = dependencies.scenes.quincho.objects.parrilla;
            parrilla.hasCarbon = true;
            return "EL carbon ya esta puesto";
        },
        'usar_encendedor_en_parrilla': (dependencies) => {
            const { scenes, gameState } = dependencies;
        ...
        'usar_carne_en_parrilla': (dependencies) => {
            const { scenes, gameState } = dependencies;
            const parrilla = scenes.quincho.objects.parrilla;
            if (gameState.currentPlayer === 'Rata') {
                if (parrilla.isLit && gameState.inventories[gameState.currentPlayer].includes('carne')) {
        ...
        'generic_description': () => {
            //console.log("Ejecutando script: generic_description " + gs.actionState.target.key + " " + gs.actionState.target.type);
            switch (gameState.actionState.target.type) {
                case 'object':
                    return gameData.objects[gameState.actionState.target.key].description;
                case 'item':
                    return gameData.items[gameState.actionState.target.key].description;
                case 'character':
                    return gameData.characters[gameState.actionState.target.key].description;
                default:
        ...
        'pancho_agarrar_mesa':() => {
            ...
            return runActionScript("generic_nopuedo");
        },
        'pancho_mirar_mesa': () => {
            console.log("Ejecutando script: pancho_mirar_mesa");
            return runActionScript("generic_description");
        },
        'pancho_mirar_parrilla_mesa': () => {
            console.log("Ejecutando script: pancho_mirar_parrilla");
            return runActionScript("generic_description");
        }
